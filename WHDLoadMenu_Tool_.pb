...
Global Prog_Title.s="WHDLoadMenu Tool"
...
Procedure Save_CSV()
  ...
  If FileSize(CSV_Path)>-1
    ...
    Select response
      ...
      Case #PB_MessageRequester_No : path=OpenFileRequester("New File", "", "Prefs File (*.prefs)|*.prefs",0)
    EndSelect 
  EndIf
  ...
  If response<>#PB_MessageRequester_Cancel And path<>""
    If CreateFile(igfile, path,#PB_Ascii)     
      ForEach IG_Database()
        output$=""
        If Short_Names
          output$+IG_Database()\IG_Short+"="
        Else
          output$+IG_Database()\IG_Name+"="
        EndIf
        If Output_Case=1  : output$=LCase(output$) : EndIf
        If Output_Case=2  : output$=UCase(output$) : EndIf
        If IG_Database()\IG_Slave<>"" : output$+IG_Database()\IG_Path+IG_Database()\IG_Slave : EndIf
        If IG_Database()\IG_Icon<>"" : output$+IG_Database()\IG_Path+IG_Database()\IG_Icon : EndIf
        If ListIndex(IG_Database())<ListSize(IG_Database()) : WriteString(igfile,output$+#LF$) : Else : WriteString(igfile,output$) : EndIf
      Next
...
Procedure Load_CSV()
  ...
  If CSV_Path<>""
    ...
    If ReadFile(CSV_File,CSV_Path,#PB_UTF8)
      ...
      Test_Line=StringField(Text_Data,1,#LF$)
      ...
      For i=1 To count
        ...
        IG_Database()\IG_Name=StringField(Text_Line,1,"=")
        IG_Database()\IG_Path=GetPathPart(StringField(Text_Line,2,"="))
        IG_Database()\IG_Genre=""
        If CountString(IG_Database()\IG_Path,"/")>1
          Backslashes=CountString(IG_Database()\IG_Path,"/")
          IG_Database()\IG_Folder=StringField(IG_Database()\IG_Path,Backslashes,"/")
        Else
          Backslashes=CountString(IG_Database()\IG_Path,":")
        ...
        EndIf
        If Prefs_Type=1 : IG_Database()\IG_Slave=GetFilePart(StringField(Text_Line,2,"=")) : EndIf
        If Prefs_Type=2 : IG_Database()\IG_Icon=GetFilePart(StringField(Text_Line,2,"="))  : EndIf
...
Procedure Fix_List()
  ...
  ForEach IG_Database()
    If Prefs_Type=1 : File=IG_Database()\IG_Slave : EndIf
    If Prefs_Type=2 : File=IG_Database()\IG_Icon : EndIf
    If FindMapElement(Comp_Map(),LCase(IG_Database()\IG_Folder+"_"+File))
      SelectElement(Comp_Database(),Comp_Map())
      IG_Database()\IG_Name=Comp_Database()\C_Name
      IG_Database()\IG_Short=Comp_Database()\C_Short
    EndIf
    If Not FindMapElement(Comp_Map(),LCase(IG_Database()\IG_Folder+"_"+File))
  ...
  Select event
    ...
    Case #PB_Event_CloseWindow
      ...
      If EventWindow()=#EDIT_WINDOW
        ...
        Define Text.s
        Define.s info_type.s
        
        If IG_Database()\IG_Slave<>"" : info_type=IG_Database()\IG_Slave : EndIf
        If IG_Database()\IG_Icon<>"" : info_type=IG_Database()\IG_Icon : EndIf
        
        If Short_Names
          Text=IG_Database()\IG_Short+Chr(10)+info_type+Chr(10)+IG_Database()\IG_Path
        Else
          Text=IG_Database()\IG_Name+Chr(10)+info_type+Chr(10)+IG_Database()\IG_Path
        EndIf
      ...
      Select gadget
        ...
        Case #EDIT_ICON
          If EventType()=#PB_EventType_Change
            IG_Database()\IG_Icon=GetGadgetText(#EDIT_ICON)
          EndIf
